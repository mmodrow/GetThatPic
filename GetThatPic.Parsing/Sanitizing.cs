...
public static class Sanitizing
{
    ...
    /// </summary>
    private static readonly Regex WhitespacePattern = new(@"\s+", RegexOptions.Multiline);

    /// <summary>
    /// The fuck pattern.
    /// </summary>
    private static readonly Regex FuckPattern = new("(f|F)[^a-zA-Z](c|[^a-zA-Z])k");

    /// <summary>
    /// The fuck capslock pattern.
    /// </summary>
    private static readonly Regex FuckCapslockPattern = new("F[^a-zA-Z](c|[^a-zA-Z])K");
        
    /// <summary>
    /// The shit pattern.
    /// </summary>
    private static readonly Regex ShitPattern = new(@"(\s|^)(s|S)(?:[^a-zA-Z\s]|h)(?:[^a-zA-Z\s]|i)(?:[^a-zA-Z\s]|t)(\s|$)");
        
    /// <summary>
    /// The shit full scramble pattern
    /// </summary>
    private static readonly Regex ShitFullScramblePattern = new(@"(\s|^)(?!\d{4})(?:[^a-zA-Z\s]{4})(\s|$)");

    /// <summary>
    /// The shit capslock pattern.
    /// </summary>
    private static readonly Regex ShitCapslockPattern = new("(S)(?:[^a-zA-Z]|H)(?:[^a-zA-Z]|I)(?:[^a-zA-Z]|T)");

    /// <summary>
    /// The bitch pattern.
    /// </summary>
    private static readonly Regex BitchPattern = new("(B|b)[^a-zA-Z]tch");

    /// <summary>
    /// The bitch capslock pattern.
    /// </summary>
    private static readonly Regex BitchCapslockPattern = new("(B|b)[^a-zA-Z]TCH");
    ...
    /// The will pattern.
    /// </summary>
    private static readonly Regex WillPattern = new(@"(\s|^)((?:[Ss]?[Hh]e|[Tt]hey|[Ww]e|I))[`´'’‘]ll");

    /// <summary>
    /// The have pattern.
    /// </summary>
    private static readonly Regex HavePattern = new(@"(\s|^)((?:[Tt]hey|[Ww]e|I|[Ss]hould))[`´'’‘]ve");

    /// <summary>
    /// The is pattern.
    /// </summary>
    private static readonly Regex IsPattern = new(@"(\s|^)((?:[Ss]?[Hh]e))[`´'’‘]s");

    /// <summary>
    /// The am pattern.
    /// </summary>
    private static readonly Regex AmPattern = new(@"(\s|^)(I)[`´'’‘]m");

    /// <summary>
    /// The are pattern.
    /// </summary>
    private static readonly Regex ArePattern = new(@"(\s|^)([Tt]hey|[Ww]e|[Yy]ou)[`´'’‘]re");

    /// <summary>
    /// The not pattern.
    /// </summary>
    private static readonly Regex NotPattern = new(@"(\s|^)([Ii]s|[Dd]oes|[Dd]o|[Dd]id|[Ww]ere|[Ww]ould|[Cc]ould|[Ss]hould)n[`´'’‘]t");

    /// <summary>
    /// The will not pattern.
    /// </summary>
    private static readonly Regex WillNotPattern = new(@"(\s|^)([Ww])on[`´'’‘]t");

    /// <summary>
    /// The cannot pattern.
    /// </summary>
    private static readonly Regex CannotPattern = new(@"(\s|^)([Cc])an[`´'’‘]t");
    ...
    /// </summary>
    private static readonly Regex CharactersNeedingSubstitutionPattern = new("[^a-zA-Z0-9_-]");

    /// <summary>
    /// The characters to drop pattern.
    /// </summary>
    private static readonly Regex CharactersToDropPattern = new("[´`'\"’.‘]");

    /// <summary>
    /// Superfluous replacement character pattern..
    /// </summary>
    private static readonly Regex SubstituteGroupsPattern = new(CharactersNeedingSubstitutionReplacement + "{2,}");

    /// <summary>
    /// The edge substitute pattern.
    /// </summary>
    private static readonly Regex EdgeSubstitutePattern = new("^" + CharactersNeedingSubstitutionReplacement + "+|" + CharactersNeedingSubstitutionReplacement + "+$");
    ...
    /// </value>
    public static string CurrentUnixTime => DateTime.UtcNow.Subtract(UnixEpoch).TotalSeconds
        .ToString(CultureInfo.InvariantCulture).Substring(0, 10);
