...
public class  DomElementAccessor : IContentAccessor
{
    ...
    public IList<string> GetContent(HtmlDocument doc, string url = null)
    {
        ...
        IList<string> output = Type switch
        {
            TargetType.Html => nodes.Select(node => node.InnerHtml).ToList(),
            TargetType.Text => nodes.Select(node => node.InnerText).ToList(),
            TargetType.Attribute => nodes.Select(node =>
                    node?.Attributes?.FirstOrDefault(attribute => AttributeName == attribute?.Name).Value)
                .ToList(),
            TargetType.Url => new List<string>
            {
